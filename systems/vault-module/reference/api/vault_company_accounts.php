<?php
/**
 * Vault Module — Company Accounts CRUD with password encryption.
...
if ($method === 'GET') {
    ...
    $res = mysqli_query($dbnew,
        "SELECT id, name, username, password_enc, link, notes, is_private, created_by, sort_order, updated_at
         FROM vault_company_accounts ORDER BY sort_order ASC, id ASC"
    );
...
function loadRowOrDeny(mysqli $db, int $id, int $currentUserId, bool $isAdmin): ?array {
    $stmt = mysqli_prepare($db, "SELECT id, password_enc, is_private, created_by FROM vault_company_accounts WHERE id = ?");
...
if ($action === 'reveal') {
    ...
    $row = loadRowOrDeny($dbnew, $id, $currentUserId, $isAdmin);
...
if ($action === 'upsert') {
    ...
    if ($id > 0) {
        ...
        $existing = loadRowOrDeny($dbnew, $id, $currentUserId, $isAdmin);
        ...
        if ($password !== '') {
            ...
            $stmt = mysqli_prepare($dbnew,
                "UPDATE vault_company_accounts SET name=?, username=?, password_enc=?, link=?, notes=?, is_private=?, sort_order=? WHERE id=?"
            );
            mysqli_stmt_bind_param($stmt, 'sssssiii', $name, $username, $passEnc, $link, $notes, $isPrivate, $sort, $id);
        } else {
            $stmt = mysqli_prepare($dbnew,
                "UPDATE vault_company_accounts SET name=?, username=?, link=?, notes=?, is_private=?, sort_order=? WHERE id=?"
            );
        ...
        $stmt = mysqli_prepare($dbnew,
            "INSERT INTO vault_company_accounts (name, username, password_enc, link, notes, is_private, created_by, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        );
...
if ($action === 'delete') {
    $id = (int)($_POST['id'] ?? 0);
    if ($id <= 0) { http_response_code(400); echo json_encode(['error' => 'id required']); exit; }
    if (!loadRowOrDeny($dbnew, $id, $currentUserId, $isAdmin)) { http_response_code(403); echo json_encode(['error' => 'forbidden']); exit; }
    $stmt = mysqli_prepare($dbnew, "DELETE FROM vault_company_accounts WHERE id = ?");
    mysqli_stmt_bind_param($stmt, 'i', $id);
    $ok = mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
...
if ($action === 'reorder') {
    ...
    $stmt = mysqli_prepare($dbnew, "UPDATE vault_company_accounts SET sort_order = ? WHERE id = ?");
