# Block: Testimonials (Animated Columns)

Three vertical columns of testimonial cards that auto-scroll at different speeds, with a fade mask top/bottom. Header section with eyebrow badge, heading, and subtitle. Responsive: 1 column on mobile, 2 on tablet, 3 on desktop.

**Preview:** animated marquee-style testimonial wall.
...
| Deps | `motion` |
| Data shape | `{ text, image, name, role }[]` |
| Responsive | 1 col mobile → 2 col `md` → 3 col `lg` |
| States | Static (no loading/empty/error — data passed as props) |
| Assets | Avatar images (randomuser.me or Unsplash) |
